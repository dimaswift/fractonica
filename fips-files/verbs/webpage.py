...
def deploy_webpage(fips_dir, proj_dir, api, webpage_dir) :
    wasm_deploy_dir = util.get_deploy_dir(fips_dir, 'libs-samples', get_build_config(api))
...
def build_deploy_webpage(fips_dir, proj_dir, api, rebuild) :
    ...
    proj_build_dir = util.get_deploy_root_dir(fips_dir, 'libs-samples')
    webpage_dir = '{}/libs-webpage-{}'.format(proj_build_dir, api)
    if rebuild :
